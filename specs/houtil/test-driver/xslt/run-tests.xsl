...
<xsl:stylesheet 
    ...
    xmlns:c="http://www.w3.org/2010/09/qt-fots-catalog"
    ...
    <!-- This is a test driver, written in XSLT, that runs XPath tests in QT3 test suite format. -->
    
    <!-- The current version is suitable for use with the tests in the HOUTIL test suite; it does not
         handle all options appearing in the main QT3 test suite. -->
    
    <!-- It runs XPath tests only; tests requiring XQuery are skipped. -->
    
    <!-- An overlay stylesheet defining processor-specific actions may be needed -->
    
    <xsl:import href="assert.xsl"/>
    <xsl:use-package name="http://expath.org/ns/houtil"/>
    ...
    <xsl:template match="/">
        <xsl:variable name="report">
            <r:test-suite-result>
                <r:test-run dateRun="{current-date()}" testsuiteVersion="1.0"/>
    ...
    <xsl:template match="c:test-case" mode="run-test" as="map(*)">
        ...
        <xsl:try>
            ...
            <xsl:catch errors="*">
                <xsl:message>EVALUATION FAILURE: {$err:code} {$err:description}</xsl:message>
